from yahtzee import Yahtzee


# These unit tests can be run using the py.test framework
# available from http://pytest.org/

def test_chance_scores_sum_of_all_dice():
    expected = 15
    actual = Yahtzee(2, 3, 4, 5, 1).chance()
    assert expected == actual
    assert 16 == Yahtzee(3, 3, 4, 5, 1).chance()


def test_yahtzee_scores_50():
    expected = 50
    actual = Yahtzee(4, 4, 4, 4, 4).yahtzee()
    assert expected == actual
    assert 50 == Yahtzee(6, 6, 6, 6, 6).yahtzee()
    assert 0 == Yahtzee(6, 6, 6, 6, 3).yahtzee()


def test_1s():
    assert Yahtzee(1, 2, 3, 4, 5).ones() == 1
    assert 2 == Yahtzee(1, 2, 1, 4, 5).ones()
    assert 0 == Yahtzee(6, 2, 2, 4, 5).ones()
    assert 4 == Yahtzee(1, 2, 1, 1, 1).ones()


def test_2s():
    assert 4 == Yahtzee(1, 2, 3, 2, 6).twos()
    assert 10 == Yahtzee(2, 2, 2, 2, 2).twos()


def test_threes():
    assert 6 == Yahtzee(1, 2, 3, 2, 3).threes()
    assert 12 == Yahtzee(2, 3, 3, 3, 3).threes()


def test_fours_test():
    assert 12 == Yahtzee(4, 4, 4, 5, 5).fours()
    assert 8 == Yahtzee(4, 4, 5, 5, 5).fours()
    assert 4 == Yahtzee(4, 5, 5, 5, 5).fours()


def test_fives():
    assert 10 == Yahtzee(4, 4, 4, 5, 5).fives()
    assert 15 == Yahtzee(4, 4, 5, 5, 5).fives()
    assert 20 == Yahtzee(4, 5, 5, 5, 5).fives()


def test_sixes_test():
    assert 0 == Yahtzee(4, 4, 4, 5, 5).sixes()
    assert 6 == Yahtzee(4, 4, 6, 5, 5).sixes()
    assert 18 == Yahtzee(6, 5, 6, 6, 5).sixes()


def test_one_pair():
    assert 6 == Yahtzee(3, 4, 3, 5, 6).score_pair()
    assert 10 == Yahtzee(5, 3, 3, 3, 5).score_pair()
    assert 12 == Yahtzee(5, 3, 6, 6, 5).score_pair()


def test_two_pair():
    assert 16 == Yahtzee(3, 3, 5, 4, 5).two_pair()
    assert 0 == Yahtzee(3, 3, 5, 5, 5).two_pair()


def test_three_of_a_kind():
    assert 9 == Yahtzee(3, 3, 3, 4, 5).three_of_a_kind()
    assert 15 == Yahtzee(5, 3, 5, 4, 5).three_of_a_kind()
    assert 0 == Yahtzee(3, 3, 3, 3, 5).three_of_a_kind()


def test_four_of_a_knd():
    assert 12 == Yahtzee(3, 3, 3, 3, 5).four_of_a_kind()
    assert 20 == Yahtzee(5, 5, 5, 4, 5).four_of_a_kind()
    assert 0 == Yahtzee(3, 3, 3, 3, 3).four_of_a_kind()


def test_small_straight():
    assert 15 == Yahtzee(1, 2, 3, 4, 5).small_straight()
    assert 15 == Yahtzee(2, 3, 4, 5, 1).small_straight()
    assert 0 == Yahtzee(1, 2, 2, 4, 5).small_straight()


def test_large_straight():
    assert 20 == Yahtzee(6, 2, 3, 4, 5).large_straight()
    assert 20 == Yahtzee(2, 3, 4, 5, 6).large_straight()
    assert 0 == Yahtzee(1, 2, 2, 4, 5).large_straight()


def test_full_house():
    assert 18 == Yahtzee(6, 2, 2, 2, 6).full_house()
    assert 0 == Yahtzee(2, 3, 4, 5, 6).full_house()


